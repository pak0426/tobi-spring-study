...
RuntimeException은 Exception의 서브클래스이므로 Exception의 일종이긴 하지만 자바는 이 RuntimeException과 그 서브클래스는 특별하게 다룬다. 아래 그림은 Exception 클래스의 서브클래스를 체크 예외와 언체크 예외로 구분한 것이다.
